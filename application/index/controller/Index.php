...
//            $voucher = $param['voucher'];
    ...
    public function goods(){
        ...
        $id = $this->app->request->param('id');
        if(!$id) msg('无效参数',2,'/index/home/index');
        
        $type = $this->app->request->param('type');
        if(!$type) msg('无效参数',2,'/index/home/index');

        $this->info = Db::name('LcProduct')->find($id);
        if(!$this->info) msg('产品不存在',2,'/index/home/index');
        if($this->info['iskq']==0) msg('该产品没有开启,无法访问',2,'/index/home/index');
        ...
        if(empty($ttimes)) msg('非交易时间，请选择其它商品！',2,'/index/home/index');
        //var_dump($this->info['opentime_'.$weekday],$weekday);die;
        if(!empty($ttimes)){
            $optime=0;
            $ttimesarr=explode("|",$ttimes);
        ...
        //查找所有的产品
        $all = Db::name('lc_product')->order("sort asc,id desc")->select();
        $this->assign('all',$all);
    ...
    public function ajaxdata(){
        ...
        foreach( $product as $k=>$val) {
            $rd = rand(-3,3);
            //修改前端显示位数！！！
            $product[$k]['Price'] =round($val['Price'],8);
    ...
    public function order()
    {
        ...
        foreach ($orderlist as $k => $v) {
            ...
            if($nowtime >= $v['selltime']){
                ...
                if($v['type'] == 1)
                {
                    addFinance($uid, $u_add, 1, '订单['.$v['orderno'].']平仓获得金额 ' . $u_add . 'BG',1024);
                    setNumber('LcUser', 'money', $u_add, 1, "id = $uid");
                }else{
                    addFinance($uid, $u_add, 1, '订单['.$v['orderno'].']平仓获得金额 ' . $u_add . 'BGT',1025);
                    setNumber('LcUser', 'bgb_money', $u_add, 1, "id = $uid");
                }
                ...
                if($v['pid'] == 15)
                {
                   ...
                   $o_log['user_money'] = Db::name('LcUser')->where('id',$uid)->value('bgb_money');
                }
