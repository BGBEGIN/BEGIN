...
class User extends Base
{
    ...
    public function hold(){
        ...
        if(!$addr)  msg('请链接钱包后再来操作',2,'/index/home/index');
    ...
    public function addorder()
    {
        ...
        $addr = $this->app->session->get('addr');
        $uid_list = Db::name('LcUser')->where(['phone'=>$addr])->find();
        $uid = $uid_list['id'];
        //用户信息
        $user = Db::name('LcUser')->find($uid);
        //验证用户是否被冻结
        ...
        if(getinfo('order_max_amount') > 0){
            ...
            if($allfee+$data['order_price'] > getinfo('order_max_amount')){
                return WPreturn(la('b137').getinfo('order_max_amount').'！',-1);
            }
        }
        ...
        if($data['order_price'] > getinfo('order_max')){
            return WPreturn(la('b47').getinfo('order_max').'！',-1);
        }
        if($data['order_price'] < getinfo('order_min')){
            return WPreturn(la('b48').getinfo('order_min').'！',-1);
        }
        ...
        if($data['type'] == 1)
        {
           if($user['money'] < $data['order_price'] + $web_poundage){
            	return WPreturn(b('96'),-1);
            } 
        }
        if($data['type'] == 2)
        {
            if($user['bgb_money'] < $data['order_price'] + $web_poundage){
            	return WPreturn(b('96'),-1);
            }
        }
        ...
        if($data['type'] == 1)
        {
             ...
             $adddata['commission'] = $user['bgb_money'] - $allfee;
        }
        ...
        if($id){
            ...
            if($moneyfrom==1){
                if($data['type'] == 1)
                {
                    addFinance($uid, $allfee, 2, '下单['.$adddata['orderno'].']从BG账户扣除金额 ' . $allfee . '',1026);
            	    setNumber('LcUser', 'money', $allfee, 2, "id = $uid");
                }else{
                    addFinance($uid, $allfee, 2, '下单['.$adddata['orderno'].']从BGT账户扣除金额 ' . $allfee . '',1027);
            	    setNumber('LcUser', 'bgb_money', $allfee, 2, "id = $uid");
                }
